...
const getData = async()=>{
  try {
  const response = await Axios.get(`http://localhost:5000/listing/${id}/edit`)
         setEdit(prevState => ({
          ...prevState,
...
useEffect(()=>{
  ...
  return (
    ...
    <form className="offset-3 col-6" onSubmit={handelSubmit} encType="multipart/form-data">
      ...
      <label htmlFor="exampleInputEmail3" className="form-label">
        ...
        <img src={edit.image.url} className="card-img-top" alt="loading..."/>
         </div>       
      </div>
      <div className="mb-3 ">
        <label htmlFor="exampleInputEmail3" className="form-label">
